...
class QKData(with_metaclass(MetaQKData, AbstractDataBase)):
    ...
    def start(self):
        ...
        if self.interval < 1440 and timeClose > timeMarketNow and timeMarketNow.time() < self.p.sessionend:  # Для внутреднивных баров если время закрытия бара еще не наступило на бирже, и сессия еще не закончилась
    ...
    def _load(self):
        ...
        if self.newCandleSubscribed:  # Если получаем новые бары по подписке
            ...
            if len(self.jsonBars) == 0:  # Если исторических данных нет
                return None  # то будем заходить еще

            self.jsonBar = self.jsonBars[self.barId]  # Получаем бар из истории
        ...
        if not self.p.LiveBars:  # Если получили все бары из истории, новые бары не принимаем
        ...
        if not self.newCandleSubscribed:  # Если принимаем новые бары и еще не подписались на них
            ...
            self.newCandleSubscribed = True  # Подписались на получение новых баров
            timeOpen = self.p.tz.localize(dt)  # Биржевое время открытия бара
    ...
    def OnNewCandle(self, data):
        ...
        if jsonData['class'] != self.classCode or jsonData['sec'] != self.secCode:  # Если бар по другому тикеру
        ...
        if self.lines.datetime[-1] >= date2num(dt):  # Если получили старый бар
            return   # то выходим, дальше не продолжаем
